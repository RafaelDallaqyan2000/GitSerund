...
const StudentEditorComponent = ({
  ...
  const blocksFromHTML = convertFromHTML(studentActionText);
  ...
  const onChange = (editorState) => {
    ...
    // setEditorState(e);
    // let html = stateToHTML(e.getCurrentContent(), e.getCurrentInlineStyle());
    // setHtml(html);

    handleFormChange("studentActionText", htmlText);
  };
      ...
      <div>
        {files.map((file, index) => {
          return file.upLoading ? (
            <p key={file.file.name}>{t("loading")}</p>
          ) : (
            <div
              key={file.file.name}
...
const mapStateToProps = (state, ownProps) => {
  return {
    ...
    studentActionText: state.formReducer.studentActionText ?? "<p> </p>",
